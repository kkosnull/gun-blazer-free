...
public class LaserBeam : MonoBehaviour {
	...
	void Update () {
		...
		else if (gameController.beam_flag==0)
		{
			...
			if (hit[i].transform.tag=="Enemy_pool" )
			{
				...
				if (hit[i].transform.position.z<17)
				{
					if (hit[i].transform.GetComponent <DestroyByContact_pooled>())
					{
						...
						gameController.explode_end_effect(hit[i].transform.position, this.transform.rotation);
					}
			...
			else if (hit[i].transform.tag=="Enemy_turret" )
			{
				...
				if (hit[i].transform.position.z<17)
				{
					 if (hit[i].transform.GetComponent <DestroyByContact_turret>())
					{
						...
						gameController.explode_end_effect(hit[i].transform.position+brickPosition, this.transform.rotation);
					}
