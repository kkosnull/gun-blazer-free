...
public class GUIBarScript : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (Background != null)
		{
			GUI.DrawTexture(new Rect(Screen.width - Position.x,Screen.height- Position.y,Background.width * ScaleSize,Background.height * ScaleSize),Background);
		}

		GUI.DrawTexture(new Rect(Screen.width - Position.x,Screen.height - Position.y,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),ValueBar);

		//if you don't have a foreground texture i won't draw it
		if (Foreground != null)
		{
			GUI.DrawTexture(new Rect(Screen.width - Position.x,Screen.height - Position.y,Foreground.width * ScaleSize,Foreground.height * ScaleSize),Foreground);
		}
		...
		if (DisplayText)
		{
			...
			GUI.Label(new Rect(Screen.width - Position.x + TextOffset.x,Screen.height - Position.y + TextOffset.y,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),TextString,LabelStyle);
